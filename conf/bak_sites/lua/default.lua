...
for key, val in pairs(args) do
    if type(val) == "table" then
        ...
        for keySub, valSub in pairs(val) do
            if valSub then
                table.insert( tmpTable, key .."="..tostring(valSub))
            else
        ...
        if val then
            table.insert( paramTable, key .."="..tostring(val))
        else
            table.insert( paramTable, key)
        end
    end
end
...
function main()
    ...
    if(isImage(url) or isFont(url)  or isMedia(url)) then
        if not no_log then
    ...
    local cdnImg = (ngx.var.args and  string.find(ngx.var.args,"imageView") ~= nil and string.find(ngx.var.proxy_host,"cdn") ~= nil)
    if cdnImg then
        if not no_log then
            ngx.log(ngx.ERR,"静态图片,字体资源:"..ngx.var.request_filename.."?"..ngx.var.args);
        end
        local cache_file = ngx.var.request_filename.."?"..ngx.var.args
        ngx.header['Content-Type'] = "image/png"
        if file_exists(cache_file) then
            local t= io.open(cache_file,'rb')
            local body = t:read("*all")
            io.close(t)
            ngx.say(body)
        else
            local res = getRes("/myproxy"..url.."?"..ngx.var.args,{ args = args ,host = ngx.var.proxy_host},cache_file)
            if res then
                local body = res.body;
                if not use_curl and string.len(body) > 20 then
                    saveToFile(ngx.var.request_filename,body)
                end
                ngx.say(body)
            else
    ...
    if res and res.body and string.len(res.body) > 0 then
        local body = mySubFilterContent(res.body,default_sub_filter_file);
        if cache_dynamic_link and string.len(body) > 20 then
